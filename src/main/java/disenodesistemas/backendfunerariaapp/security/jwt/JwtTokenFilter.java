...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    try {
      if (jwtTokenExist(request)) {
        ...
        if (nonNull(claims.get(authorities))
            && Boolean.TRUE.equals(jwtProvider.validateToken(getToken(request)))) {
  ...
  private Claims validateToken(final HttpServletRequest request) {
    final String jwtToken =
        request.getHeader(header).replace(prefix + StringUtils.SPACE, StringUtils.EMPTY);
  ...
  private void setUpSpringAuthentication(final Claims claims) {
    ...
    final UsernamePasswordAuthenticationToken auth =
        new UsernamePasswordAuthenticationToken(claims.getSubject(), null, authoritiesCollection);
  ...
  private boolean jwtTokenExist(final HttpServletRequest request) {
    ...
    return nonNull(authenticationHeader)
        && authenticationHeader.startsWith(prefix + StringUtils.SPACE);
  }
}
